...
CREATE OR REPLACE TASK dvd_rental.bronze.category_copy_task
  ...
  SCHEDULE  = '5 MINUTE'
AS
COPY INTO dvd_rental.bronze.category (category_id, category_name, last_update, file_name, load_timestamp)
FROM (
  SELECT
    $1 AS category_id,
    $2 AS category_name,
...
CREATE OR REPLACE TASK dvd_rental.bronze.film_category_copy_task
  ...
  SCHEDULE  = '5 MINUTE'
AS
COPY INTO dvd_rental.bronze.film_category (film_id, category_id, last_update, file_name, load_timestamp)
FROM (
  SELECT
    $1 AS film_id,
    $2 AS category_id,
...
COPY INTO dvd_rental.bronze.film (
  ...
  last_update, special_features, fulltext, file_name, load_timestamp
)
FROM (
  SELECT
    $1 AS film_id,
...
CREATE OR REPLACE TASK dvd_rental.bronze.payment_copy_task
  ...
  SCHEDULE  = '5 MINUTE'
AS
COPY INTO dvd_rental.bronze.payment (payment_id, customer_id, staff_id, rental_id, amount, payment_date, file_name, load_timestamp)
FROM (
  SELECT
    $1 AS payment_id,
    $2 AS customer_id,
...
CREATE OR REPLACE TASK dvd_rental.bronze.film_actor_copy_task
  ...
  SCHEDULE  = '5 MINUTE'
AS
COPY INTO dvd_rental.bronze.film_actor (actor_id, film_id, last_update, file_name, load_timestamp)
FROM (
  SELECT
    $1 AS actor_id,
    $2 AS film_id,
...
COPY INTO dvd_rental.bronze.customer (
  ...
  activebool, create_date, last_update, active, file_name, load_timestamp
)
FROM (
  SELECT
    $1 AS customer_id,
...
COPY INTO dvd_rental.bronze.address (
  ...
  last_update, file_name, load_timestamp
)
FROM (
  SELECT
    $1 AS address_id,
...
COPY INTO dvd_rental.bronze.staff (
  ...
  active, username, staff_password, last_update, picture, file_name, load_timestamp
)
FROM (
  SELECT
    $1 AS staff_id,
...
CREATE OR REPLACE TASK dvd_rental.bronze.city_copy_task
  ...
  SCHEDULE  = '5 MINUTE'
AS
COPY INTO dvd_rental.bronze.city (city_id, city, country_id, last_update, file_name, load_timestamp)
FROM (
  SELECT
    $1 AS city_id,
    $2 AS city,
...
CREATE OR REPLACE TASK dvd_rental.bronze.country_copy_task
  ...
  SCHEDULE  = '5 MINUTE'
AS
COPY INTO dvd_rental.bronze.country (country_id, country, last_update, file_name, load_timestamp)
FROM (
  SELECT
    $1 AS country_id,
    $2 AS country,
...
CREATE OR REPLACE TASK dvd_rental.bronze.actor_copy_task
  ...
  SCHEDULE  = '5 MINUTE'
AS
COPY INTO dvd_rental.bronze.actor (actor_id, first_name, last_name, last_update, file_name, load_timestamp)
FROM (
  SELECT
    $1 AS actor_id,
    $2 AS first_name,
...
COPY INTO dvd_rental.bronze.rental (
  rental_id, rental_date, inventory_id, customer_id, return_date, staff_id, last_update, file_name, load_timestamp
)
FROM (
  SELECT
    $1 AS rental_id,
...
CREATE OR REPLACE TASK dvd_rental.bronze.language_copy_task
  ...
  SCHEDULE  = '5 MINUTE'
AS
COPY INTO dvd_rental.bronze.language (language_id, language_name, last_update, file_name, load_timestamp)
FROM (
  SELECT
    $1 AS language_id,
    $2 AS language_name,
...
CREATE OR REPLACE TASK dvd_rental.bronze.inventory_copy_task
  ...
  SCHEDULE  = '5 MINUTE'
AS
COPY INTO dvd_rental.bronze.inventory (inventory_id, film_id, store_id, last_update, file_name, load_timestamp)
FROM (
  SELECT
    $1 AS inventory_id,
    $2 AS film_id,
...
CREATE OR REPLACE TASK dvd_rental.bronze.store_copy_task
  ...
  SCHEDULE  = '5 MINUTE'
AS
COPY INTO dvd_rental.bronze.store (store_id, manager_staff_id, address_id, last_update, file_name, load_timestamp)
FROM (
  SELECT
    $1 AS store_id,
